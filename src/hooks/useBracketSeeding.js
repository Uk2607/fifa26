...
export function useBracketSeeding(qualificationState, allocatedThirds, koMatches) {
  ...
  const r32MatchesSeeding = useMemo(() => {
    ...
    return {
      ...
      74: { t1: auto["C"]?.[0] || "1C", t2: auto["F"]?.[1] || "2F" },

      75: { t1: auto["E"]?.[0] || "1E", t2: allocatedThirds[75] || "3ABCDF" },
      76: { t1: auto["F"]?.[0] || "1F", t2: auto["C"]?.[1] || "2C" },
      77: { t1: auto["E"]?.[1] || "2E", t2: auto["I"]?.[1] || "2I" },

      78: { t1: auto["I"]?.[0] || "1I", t2: allocatedThirds[78] || "3CDFGH" },
      79: { t1: auto["A"]?.[0] || "1A", t2: allocatedThirds[79] || "3CEHFI" },
      80: { t1: auto["L"]?.[0] || "1L", t2: allocatedThirds[80] || "3EHIJK" },

      81: { t1: auto["G"]?.[0] || "1G", t2: allocatedThirds[81] || "3AEHIJ" },
      82: { t1: auto["D"]?.[0] || "1D", t2: allocatedThirds[82] || "3BEFIJ" },

      83: { t1: auto["H"]?.[0] || "1H", t2: auto["J"]?.[1] || "2J" },
      84: { t1: auto["K"]?.[1] || "2K", t2: auto["L"]?.[1] || "2L" },
      85: { t1: auto["B"]?.[0] || "1B", t2: allocatedThirds[85] || "3EFGIJ" },
      86: { t1: auto["D"]?.[1] || "2D", t2: auto["G"]?.[1] || "2G" },

      87: { t1: auto["J"]?.[0] || "1J", t2: auto["H"]?.[1] || "2H" },
      88: { t1: auto["K"]?.[0] || "1K", t2: allocatedThirds[88] || "3DEIJL" }
    };
  ...
  const roundOf16Seeding = useMemo(() => {
    ...
    return {
      89: { t1: getWinner(m[73].t1, m[73].t2, 73, koMatches) || "Winner Match 73", t2: getWinner(m[74].t1, m[74].t2, 74, koMatches) || "Winner Match 74" },
      90: { t1: getWinner(m[75].t1, m[75].t2, 75, koMatches) || "Winner Match 75", t2: getWinner(m[76].t1, m[76].t2, 76, koMatches) || "Winner Match 76" },
      91: { t1: getWinner(m[77].t1, m[77].t2, 77, koMatches) || "Winner Match 77", t2: getWinner(m[78].t1, m[78].t2, 78, koMatches) || "Winner Match 78" },
      92: { t1: getWinner(m[79].t1, m[79].t2, 79, koMatches) || "Winner Match 79", t2: getWinner(m[80].t1, m[80].t2, 80, koMatches) || "Winner Match 80" },

      93: { t1: getWinner(m[81].t1, m[81].t2, 81, koMatches) || "Winner Match 81", t2: getWinner(m[82].t1, m[82].t2, 82, koMatches) || "Winner Match 82" },
      94: { t1: getWinner(m[83].t1, m[83].t2, 83, koMatches) || "Winner Match 83", t2: getWinner(m[84].t1, m[84].t2, 84, koMatches) || "Winner Match 84" },
      95: { t1: getWinner(m[85].t1, m[85].t2, 85, koMatches) || "Winner Match 85", t2: getWinner(m[86].t1, m[86].t2, 86, koMatches) || "Winner Match 86" },
      96: { t1: getWinner(m[87].t1, m[87].t2, 87, koMatches) || "Winner Match 87", t2: getWinner(m[88].t1, m[88].t2, 88, koMatches) || "Winner Match 88" },
    };
  ...
  const quarterFinalsSeeding = useMemo(() => {
    ...
    return {
      ...
      98: { t1: getWinner(r[91].t1, r[91].t2, 91, koMatches) || "Winner Match 91", t2: getWinner(r[92].t1, r[92].t2, 92, koMatches) || "Winner Match 92" },

      99: { t1: getWinner(r[93].t1, r[93].t2, 93, koMatches) || "Winner Match 93", t2: getWinner(r[94].t1, r[94].t2, 94, koMatches) || "Winner Match 94" },
